...
function index() {
  const [industryrsData, setIndustrysData] = useState<IndustryrData[]>([]);
    const [selectedIndustry, setSelectedIndustry] = useState<IndustryrData | null>(null);
  ...
  const deleteIndustry = async (industry: IndustryrData) => {
    if (!window.confirm('Are you sure you want to delete this industry?')) return;

    try {
      const response = await fetch(`http://127.0.0.1:8000/api/industry/delete/${industry.id}`, { // Use aggregator.id
        method: 'DELETE',
      });

      if (response.ok) {
        // Refresh the aggregators list after deletion
        const refreshResponse = await fetch('http://127.0.0.1:8000/api/industry/all');
        const { data } = await refreshResponse.json();
        setIndustrysData(data);
        alert('Industry deleted successfully');
      } else {
        const errorData = await response.json();
        alert(errorData.detail || 'Failed to delete industry');
      }
    } catch (error) {
  ...
  const handleIndustrySubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append('username', industryFormData.username);
      formData.append('industryname', industryFormData.industryname);
      formData.append('hashed_password', industryFormData.hashed_password);
      formData.append('email', industryFormData.email);
      formData.append('phone', industryFormData.phone);
      if (industryFormData.logo) {
        formData.append('logo', industryFormData.logo);
      }

      const response = await fetch('http://127.0.0.1:8000/api/industry/register', {
        method: 'POST',
        body: formData,
      });
  ...
  const columns = useMemo<MRT_ColumnDef<any>[]>(
    () => [
      ...
      {
        accessorKey: 'industry_name', //access nested data with dot notation
        header: 'Industry Name',
        size: 100
      },
      {
      ...
      },
      {
        ...
        Cell: ({ row }) => {
          const handleEdit = () => {
            ...
            const industryData = {
              industry_id: row.original.id,
              username: row.original.username,
              industry_name: row.original.industry_name,
            ...
            console.log('Editing aggregator:', industryData);
          
            // Set the form data
            setFormData(industryData);
          ...
          const handleDelete = () => {
            ...
            deleteIndustry(row.original); // Use row.original instead of aggregator
          };
  ...
  const table = useMaterialReactTable({
    columns,
    data: industryrsData, //data must be memoized or stable (useState, useMemo, defined outside of this component, etc.)
  ...
  return (
    <>
      <div className="flex flex-col items-center mt-8 intro-y sm:flex-row">
        <h2 className="mr-auto text-lg font-medium">Industry Lists</h2>
  ...
  ==============================*/}
      ...
      >
        ...
        <Slideover.Panel>
          ...
          <Slideover.Title>
            <h2 className="mr-auto text-base font-medium">Add Industry</h2>
          </Slideover.Title>
            ...
            <form onSubmit={handleIndustrySubmit}>
              <div>
                ...
                <FormInput
                  ...
                  value={industryFormData.username}
              ...
              <div className="mt-3">
                ...
                <FormInput
                  ...
                  value={industryFormData.hashed_password}
                  onChange={handleInputChange}
                />
              </div>
              <div className="mt-3">
              ...
              <Slideover.Footer>
                <Button variant="pending" type="button" className="w-20 mr-1" onClick={() => setIndustryFormData({
                  username: '',
                  industryname: '',
                  hashed_password: '',
                  email: '',
        ...
        <Slideover.Panel>
          ...
          <Slideover.Description>
            ...
            <div>
              ...
              <div className="mt-3">
                <FormLabel htmlFor="industry-name">Industry Name</FormLabel>
                <FormInput id="industryname" type="text" placeholder="Enter the industry name" value={formData.industryname} onChange={handleChange} />
              </div>
              {/* <div className="mt-3">
            <FormLabel htmlFor="hashed_password">Password</FormLabel>

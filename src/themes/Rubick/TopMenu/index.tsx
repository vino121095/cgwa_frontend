...
//User images
import userProfile from '@/assets/images/fakers/user.png';
//Js-cookies
import Cookies from 'js-cookie';

function Main() {
  const navigate = useNavigate();
  //useMatches for breadcrumbs
  const matches = useMatches();
  const breadcrumb:any = matches.filter((item:any)=>{
  ...
  //This is role values store place
  // State for userRoles
  const [userRoles, setUserRoles] = useState<string[]>([]); // Initialize with default roles
  const menuStore = useAppSelector(selectMenu("top-menu"));
  const menu = () =>{
    if(Cookies.get('viewsite'))
    {
      const viewsite : string | any = Cookies.get('viewsite')
      if(viewsite === "visitor")
      {
        setUserRoles(viewsite);
        return nestedMenu(menuStore, location, userRoles) || []; // add the extra parameter of userRoles
      }
    }
    return nestedMenu(menuStore, location, userRoles) || []; // add the extra parameter of userRoles
  } 
  useEffect(() => {
    ...
    {
      const roles : string | any | undefined = Cookies.get('roles');
      if (roles) {
        setUserRoles(roles);
      }
    }
    setFormattedMenu(menu());
  }, [menuStore, location.pathname, userRoles]);
            ...
            <div className="flex items-center h-[70px] z-[51] relative">
              ...
              <Link to="/" className="hidden -intro-x md:flex">
                <img
                  ...
                  // className="w-100"
                  src={logoUrl}
                  style={{ width: "180px", paddingTop: '2px' }}
                />
              ...
              >
                ...
                {breadcrumb[0]?.handle?.breadcrumbTitle}
                </Breadcrumb.Link>
              </Breadcrumb>
              {/* END: Breadcrumb */}
              {/* BEGIN: Search */}
              ...
              <Menu>
                <Menu.Button className="flex items-center">
                  <img
                    ...
                    className="w-9 h-9"
                  />
                  <div className="font-medium text-[#d7e1dd] px-1">
                      Admin
                    </div>
                </Menu.Button>
